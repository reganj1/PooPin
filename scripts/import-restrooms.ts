...
const allowedSources = new Set<BathroomSource>(["user", "google_places", "city_open_data", "partner", "other"]);
...
const parseJson = (content: string): RawRecord[] => {
  ...
  throw new Error("Unsupported JSON shape. Expected an array, FeatureCollection, or object with records/data/rows/items.");
};
...
const toImportRecord = (row: RawRecord, options: ImportOptions): ImportBathroomRecord | null => {
  const name = parseString(getValue(row, ["name", "restroom_name", "facility_name", "site_name", "location_name"]));
  const address = parseString(
    getValue(row, ["address", "street_address", "street", "location", "location_address", "cross_street"])
  );
  ...
  const sourceExternalId = sourceExternalIdRaw && sourceExternalIdRaw.length > 0 ? sourceExternalIdRaw : null;

  const city = parseString(getValue(row, ["city"])) ?? options.defaultCity;
  const state = parseString(getValue(row, ["state", "state_code", "province"])) ?? options.defaultState;

  const accessType = resolveAccessType(getValue(row, ["access_type", "access", "access_level"]));
  const requiresPurchase =
    parseBoolean(getValue(row, ["requires_purchase", "purchase_required"])) || accessType === "customer_only";
  ...
  return {
    ...
    is_gender_neutral: parseBoolean(getValue(row, ["is_gender_neutral", "gender_neutral", "all_gender"])),
    is_accessible: parseBoolean(getValue(row, ["is_accessible", "accessible", "ada_accessible"])),
    ...
    switch (arg) {
      ...
      case "--source":
        if (!allowedSources.has(next as BathroomSource)) {
          throw new Error("--source must be one of user|google_places|city_open_data|partner|other");
        }
